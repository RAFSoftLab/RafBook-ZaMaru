...
public class ApiClientUserTest extends ApplicationTest {
    ...
    public void testSearchFunctionality() {
        ...
        clickOn("Pretraga:").write("Mara Arbutina");

        ObservableList<Person> items = table.getItems();
        boolean found = items.stream().anyMatch(user ->
                user.getFirstName().contains("Mara") || user.getLastName().contains("Arbutina"));

        assertTrue(found, "Pretraga nije pronašla korisnika sa imenom 'Mara Arbutina'.");
    }
    ...
    public void testDeleteUser() {
        ...
        Node firstRow = lookup(".table-row-cell").nth(1).query();
        clickOn(firstRow);
        ...
        Button okButtonNode = (Button) lookup(".button:contains('OK')").query();
        assertNotNull(okButtonNode, "Dugme 'OK' nije pronađeno.");
        clickOn(okButtonNode);
